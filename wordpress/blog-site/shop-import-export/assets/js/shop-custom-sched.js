jQuery(function($){
    ...
    var acf_last_input = $('input[name=acf_last_input]').val();
    ...
    function minus() {
        ...
        $('.sie-custom-sched table tbody tr:last td:first').html('<p class="plus">'+plus+'</p>');
        var after_remove_cnt = $('.sie-custom-sched table tbody tr').length;
        if (after_remove_cnt < 2) {
            $('.sie-custom-sched table tbody tr td .plus').html(plus);
        }
    }
    ...
    function custom_sched_html() {
        ...
        if (items_length > 0) {
            for (let i = 0; i < items_length; i++) {
                ...
                if (items_length === cntr && items_length > 1) {
        ...
        str+='</div>';
        $(acf_last_input).after(str);
    }

    /**
     * This is the last row.
     * This row has the plus and minus
     * sign on it.
     */
    function last_row() {
        var str = '';
