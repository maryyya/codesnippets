<?php
require_once WP_CONTENT_DIR . '/plugins/html-pdf-generator/includes/tcpdf.php';

/**
 * This is was primarily created
 * to enable output Japanese
 * file name for the pdf.
 *
 * The only thing I changed was commenting out 3 lines of code.
 * 1. Line 40 : $name = preg_replace('/[\s]+/', '_', $name);
 * 2. Line 41 : $name = preg_replace('/[^a-zA-Z0-9_\.-]/', '', $name);
 * 3. Line 145: // header('Content-Disposition: attachment; filename="' . basename($name) . '"'); This I changed from basename($name) to $name.
 */
class PPC_TCPDF extends TCPDF
{
    /**
